...
public abstract class WorldGenHugeMushroomBase extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		if (!soil.getBlock().canSustainPlant(soil, world, pos.down(), EnumFacing.UP, (IPlantable) Blocks.SAPLING)) {
			return false;
		}
		...
		for (int j = 0; j<height+1; j++) {
			...
			for (int i = -1; i<2; i++) {
				for (int k = -1; k<2; k++) {
					if ((Math.abs(i)+Math.abs(k)<2)||(j==0||j==height)){
		...
		for (int i = -5; i<6; i++) {
			for (int k = -5; k<6; k++) {
				...
				if (distance < 2.5) {
					placeCap(world, rand, startPos, newPos, height, bendDir);
				} else if (distance < 5) {
					placeCap(world, rand, startPos, newPos.down(), height, bendDir);
				} else if (distance < 6) {
					placeCap(world, rand, startPos, newPos.down(2), height, bendDir);
				}
			}
		}
		return true;
