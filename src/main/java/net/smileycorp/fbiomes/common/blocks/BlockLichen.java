...
public class BlockLichen extends BlockBush implements IGrowable, BlockProperties, IMultifaceBlock<BlockLichen> {
	...
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {
		return new ItemStack(FBiomesItems.LICHEN);
	}
	
	@Override
	protected ItemStack getSilkTouchDrop(IBlockState state) {
		return new ItemStack(FBiomesItems.LICHEN, IMultifaceBlock.getFacings(state).length);
	}
	...
	public static IBlockState getBlockState(int meta) {
		return meta == 0 ? Blocks.AIR.getDefaultState() : FBiomesBlocks.LICHEN.get(meta / 16).getDefaultState().withProperty(META, meta % 16);
	}
