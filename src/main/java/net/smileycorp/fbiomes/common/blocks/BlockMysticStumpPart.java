...
public class BlockMysticStumpPart extends BlockBase {
    ...
    public BlockMysticStumpPart() {
        super("mystic_stump_part", Constants.MODID, Material.WOOD, SoundType.WOOD, 2, 5,  "axe", 0, FantasyBiomes.TAB);
        disableStats();
        setLightOpacity(0);
        translucent = true;
        fullBlock = false;
    ...
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return Item.getItemFromBlock(FBiomesBlocks.MYSTIC_STUMP);
    }
    ...
    public ItemStack getPickBlock(IBlockState state, RayTraceResult result, World world, BlockPos pos, EntityPlayer player) {
        ...
        return world.getBlockState(stump).getBlock() == FBiomesBlocks.MYSTIC_STUMP ?
                FBiomesBlocks.MYSTIC_STUMP.getPickBlock(world.getBlockState(stump), result, world, stump, player)
                : new ItemStack(FBiomesBlocks.MYSTIC_STUMP);
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if(world.isRemote) return false;
            BlockPos stump = getStumpPos(world, pos);
            if(stump != null) return FBiomesBlocks.MYSTIC_STUMP.onBlockActivated(world, stump, world.getBlockState(stump),
