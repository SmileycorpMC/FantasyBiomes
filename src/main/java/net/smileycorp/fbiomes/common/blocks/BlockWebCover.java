...
public class BlockWebCover extends BlockWeb implements BlockProperties, IMultifaceBlock<BlockWebCover> {
	...
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {
		return new ItemStack(FBiomesItems.WEB_COVER);
	}
	
	@Override
	protected ItemStack getSilkTouchDrop(IBlockState state) {
		return new ItemStack(FBiomesItems.WEB_COVER, IMultifaceBlock.getFacings(state).length);
	}
	...
	public List<BlockWebCover> getBlocks() {
		return FBiomesBlocks.WEB_COVER;
	}
	...
	public static IBlockState getBlockState(int meta) {
		return meta == 0 ? Blocks.AIR.getDefaultState() : FBiomesBlocks.WEB_COVER.get(meta / 16).getDefaultState().withProperty(META, meta % 16);
	}
