...
public class WorldGenHollowLog extends WorldGenerator implements IMultiFacePlacer {
    ...
    public boolean generate(World world, Random rand, BlockPos pos) {
        ...
        IBlockState log = PhantasiaiBlocks.WOOD.getLogState(EnumWoodType.ORANTIKKU, BlockLog.EnumAxis.fromFacingAxis(direction.getAxis()));
    ...
    private void placeDecorations(World world, Random rand, BlockPos pos, EnumFacing facing) {
        ...
        if (rand.nextBoolean() && world.isAirBlock(pos)) lichen.add(pos);
                ...
                (rand.nextInt(5) < 2 ? PhantasiaiBlocks.glowshrooms[rand.nextInt(PhantasiaiBlocks.glowshrooms.length)]:
                    PhantasiaiBlocks.shrooms[rand.nextInt(PhantasiaiBlocks.shrooms.length)]).getDefaultState().withProperty(BlockFBMushroom.FACING, facing));
    }
