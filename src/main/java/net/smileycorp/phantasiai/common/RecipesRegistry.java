...
@Mod.EventBusSubscriber(modid = "phantasiai")
public class RecipesRegistry {
	...
	public static void recipeRegisty(RegistryEvent.Register<IRecipe> event) {
		...
		registerCrafting();
		registerBrewing();
		PhantasiaiBlocks.WOOD.registerRecipes();
	}
	
	private static void registerFuel() {
		FuelHandler fuel = FuelHandler.getInstance();
		fuel.registerFuel(new ItemStack(PhantasiaiBlocks.MUD, 1, 1), 400);
	}
	
	private static void registerOredict() {
		for (BlockFBMushroom glowshroom : PhantasiaiBlocks.glowshrooms) OreDictionary.registerOre("glowshroom", glowshroom);
	}
